...
def fast_cross_entropy_loss(
    ...
    """
        Computes Cross Entropy Loss significantly faster than Hugging Face.
    """
    # I/O device checking for multi-GPU safety
    device = logits.device
    if torch.cuda.device_count() > 1:
        # Ensure we're using a CUDA device for Triton kernels
        if device.type != 'cuda':
            raise ValueError("Triton kernels require CUDA device for logits tensor")
        
        # Ensure all tensors are on the same device
        labels = labels.to(device)
        
        # Set the CUDA device for this operation
    ...
    # Go for logsumexp - we need this for numerical stability
    logsumexp = torch.empty(batch_size, dtype = torch.float32, device = logits.device)
    ...
    with torch_cuda_device(device):
        ...
        _cross_entropy_forward[(batch_size,)](
            ...
            SOFTCAP        = float(logit_softcapping) if logit_softcapping != 0 else 0,
            DO_LOGIT_SCALING= logit_scaling != 0,
            LOGIT_SCALE    = float(logit_scaling) if logit_scaling != 0 else 0,
            num_warps      = num_warps,
        )
    pass
    
    # Sum up only the right indices to avoid counting padding
    if n_items is None:
        mask = (labels != -100)
        non_ignore = mask.sum()
        return torch.where(
            mask, loss, torch.zeros_like(loss)
        ).sum() / non_ignore
    else:
        # For optimized gradient accumulation
        return torch.where(
            labels != -100, loss, torch.zeros_like(loss)
        ).sum() / n_items
    pass
pass
