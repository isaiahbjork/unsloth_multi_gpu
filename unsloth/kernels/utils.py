...
if torch.cuda.device_count() > 1:
    def torch_cuda_device(device):
        # Always return a valid CUDA context, even for CPU tensors
        if hasattr(device, 'type') and device.type == 'cuda':
            return torch.cuda.device(device)
        else:
            # If tensor is on CPU or device is unknown, use device 0
            # This fixes the "Pointer argument cannot be accessed from Triton" error
